...
function displayListOfRoutes() {
    ...
    var routeCode, routeName, routeTrailHead, trailHeadLat, trailHeadLong, routeFile, routeLength, routeDiff;
    ...
    for (var arrayIndex = 0; arrayIndex < arrayOfPOIData.length; arrayIndex++) {
        ...
        if ((trailHeadLat != 0) && (trailHeadLong != 0)) {
            ...
            //Build list item
            display = display + buildListItem(routeCode, routeName, trailHead, length, difficulty, trailHeadLat, trailHeadLong, distanceFromBase, routeFile);
...
//--------------------------------------------------------------------
function buildListItem(parPOI_ID, parPOI_Title, parTrailHead, parPOI_Length, parPOI_Difficulty, parPOI_Lat, parPOI_Long, parDistanceFromBase, parRouteFile) {

    var display = "";
    var distanceDisplay = "";

    //Build url string
    var poi_listItem_URL = "route.html?qryPOI=" + parPOI_ID +
    ...
    if ((parDistanceFromBase !== null)) {
        ...
        display += "<li>" + '<a class="list-group-item" rel="external" href="' + poi_listItem_URL + '"><span class="title">' +
                    parPOI_Title + '</span><br/><span class="subTitle">' + parTrailHead + '<br/>' + parPOI_Length + '; ' + parPOI_Difficulty + '</span><span class="distance">' + distanceDisplay + "</span></a></li>";
    }
    else {
